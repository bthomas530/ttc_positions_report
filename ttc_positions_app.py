...
# v2.2.0 - Persistent IBKR connection, working self-update, Cboe fallback
...
import app_update
from ibkr_manager import IBKRManager, IBKRUnavailableError, probe_ib_ports
...
# ============================================
# Auto-Update System (see app_update.py)
# ============================================
def check_for_updates():
    return app_update.check_for_updates(APP_VERSION, USER_AGENT)
...
def show_update_dialog(update_info):
    ...
    try:
        # JSON-encode everything injected into JS so release notes can't break
        ...
        if (typeof showUpdateNotification === 'function') {{
            showUpdateNotification({version_js}, {notes_js});
        }}
        '''
...
def show_startup_toast(message, kind='info'):
    """Show a toast in the web UI (used for post-update confirmation)."""
    if not HAS_WEBVIEW or not webview_window:
        return
    try:
        js_code = (
...
def check_updates_background():
    ...
    for message, kind in startup_messages:
...
# App settings (persisted clientId etc.)
...
ibkr = None  # IBKRManager, created in main()
qual_failure_cache = QualFailureCache(os.path.join(APP_DIR, 'qual_failures.json'))
...
# ============================================
# IBKR Connection (see ibkr_manager.py)
# ============================================
def is_market_open():
...
def apply_price_fallbacks(market_data, wanted_symbols, missing_symbols):
    """Fill missing prices: Yahoo -> Cboe -> disk cache. Mutates market_data,
    returns data_sources dict for symbols that came from a fallback."""
    data_sources = {}
    ...
    if missing:
        ...
        if still_missing:
            ...
            for symbol, sdata in cboe_data.items():
                market_data[symbol] = sdata
...
def get_ibkr_data():
    """Get position data via the persistent IBKR manager, with
    Yahoo/Cboe/cache fallback for symbols IBKR couldn't price."""
    global WATCHLIST

    if ibkr is None:
        raise IBKRUnavailableError('IBKR connection manager is not running.')
    snapshot = ibkr.get_snapshot([s for s in WATCHLIST if not is_cusip(s)])
    positions = snapshot['positions_raw']
    market_data = dict(snapshot['market_data'])

    # Symbols we want a price for: STK/OPT positions + watchlist
...
def _serve_external_fallback(reason):
    """Build the Yahoo/Cboe + cache fallback response.
    Returns (json_dict, status_code) or (None, None)."""
    try:
        ...
        if 'yahoo' in live_sources or 'cboe' in live_sources:
        ...
        if ibkr is not None:
            retry_in = ibkr.retry_in_seconds()
            if retry_in > 0:
...
def get_data():
    ...
    try:
        ...
        # Typed connection failure: the manager keeps reconnecting in the
        # background; serve external/cached data in the meantime.
...
def build_diagnostics():
    """Run a fresh probe and assemble the diagnostics payload from manager state."""
    probes = probe_ib_ports()
    open_endpoints = [p for p in probes if p['reachable']]

    status = ibkr.status() if ibkr is not None else {}
...
def api_download_update():
    """API endpoint to download, verify, and install an update"""
    global pending_update
    ...
    try:
        ...
        if not app_update.verify_download(
                download_path,
                pending_update['asset_name'],
                pending_update.get('checksums_url'),
                USER_AGENT):
            try:
    ...
    <div class="container">
        <header class="header">
            <div class="header-top">
                <div class="brand">
                    <h1>TTC Positions</h1>
                    <span class="version-badge">v__APP_VERSION__</span>
                </div>
        ...
        <footer class="footer">
            <span>TTC Positions Report v__APP_VERSION__</span>
...
def main():
    ...
    # Post-update bookkeeping: detect a version change and failed updates
    ...
    if previous_version and app_update.parse_version(previous_version) < app_update.parse_version(APP_VERSION):
        ...
        startup_messages.append((f'Updated to v{APP_VERSION}', 'success'))
    ...
    save_version_info(APP_VERSION)

    # Ensure resources exist
    ensure_resources()

    # Start the persistent IBKR connection manager
    settings = load_settings()
    ibkr = IBKRManager(
        client_id=settings.get('ibkr_client_id'),
        on_client_id_change=lambda cid: save_setting('ibkr_client_id', cid),
        qual_failure_cache=qual_failure_cache,
    )
    ibkr.start()
    logger.info(f'IBKR connection manager started (clientId={ibkr.client_id})')
